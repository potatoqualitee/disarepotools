function Uninstall-DisaPatch {
    ...
    .DESCRIPTION
        ...
        Note that sometimes, an uninstall will leave registry entries and Get-DisaInstalledSoftware will report the product is installed. This is the behavior of some patches and happens even when using the Windows uninstall GUI.
    ...
    .PARAMETER InputObject
        Allows results to be piped in from Get-DisaInstalledSoftware
    ...
    .EXAMPLE
        PS C:\> Uninstall-DisaPatch -ComputerName sql2017 -HotfixId kb4498951 -Confirm:$false

        Uninstalls kb4498951 on sql2017 without prompts

    .EXAMPLE
        PS C:\>  Get-DisaInstalledSoftware -ComputerName server23, server24 -Pattern kb4498951 | Uninstall-DisaPatch

        Uninstalls kb4498951 from server23 and server24

    .EXAMPLE
        PS C:\> Uninstall-DisaPatch -ComputerName sql2017 -HotfixId KB4534273 -WhatIf

        Shows what would happen if the command were to run but does not execute any changes

    .EXAMPLE
        PS C:\> Install-DisaPatch -ComputerName sql2017 -FilePath \\dc\sql\windows10.0-kb4486129-x64_0b61d9a03db731562e0a0b49383342a4d8cbe36a.msu
        PS C:\> Get-DisaInstalledSoftware -Pattern kb4486129 -ComputerName sql2017 | Uninstall-DisaPatch
...
#>
    ...
    process {
        if (-not $PSBoundParameters.HotfixId -and -not $PSBoundParameters.InputObject.HotfixId) {
            ...
            Stop-PSFFunction -EnableException:$EnableException -Message "You must specify either HotfixId or pipe in the results from Get-DisaInstalledSoftware"
            return
        }
        ...
        foreach ($hotfix in $HotfixId) {
            ...
            foreach ($computer in $PSBoundParameters.ComputerName) {
                $exists = Get-DisaInstalledSoftware -Pattern $hotfix -ComputerName $computer -IncludeHidden
                if (-not $exists) {
