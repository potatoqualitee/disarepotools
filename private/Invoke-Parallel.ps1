function Invoke-Parallel {
    ...
    begin {
        ...
        if ( -not $PSBoundParameters.ContainsKey('MaxQueue') ) {
            if ($RunspaceTimeout -ne 0) { $global:MaxQueue = $Throttle }
            else { $global:MaxQueue = $Throttle * 3 }
        } else {
            $global:MaxQueue = $MaxQueue
        }
            ...
            Do {
                ...
                if (-not $Quiet) {
                    $writeProgressSplat = @{
                        ...
                        Activity         = $Activity
                        Status           = $Status
                        CurrentOperation = "$startedCount threads defined - $totalCount $ObjectName - $global:completedCount $ObjectName processed"
                        PercentComplete  = try { $global:completedCount / $totalCount * 100 } catch { 0 }
                    }
    ...
    process {
        ...
        try {
            ...
            foreach ($object in $allObjects) {
                ...
                $firstRun = $true
                while ($runspaces.count -ge $global:MaxQueue) {
                    #give verbose output
                    if ($firstRun) {
                        Write-Verbose "$($runspaces.count) items running - exceeded $global:MaxQueue limit."
                    }
                    $firstRun = $false
